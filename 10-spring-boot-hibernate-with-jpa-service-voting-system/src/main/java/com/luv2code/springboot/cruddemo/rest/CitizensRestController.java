...
public class CitizensRestController {
	...
	public Citizens addEmployee(@RequestBody Citizens theCitizens) {
	...
	// this is force the hibernate session to save new item instead of making update on it 	
		theCitizens.setCitizen_pessel(0);//if the id null or 0 DAO will insert new customer 
		
		
		citizensService.save(theCitizens);
...
//	
	...
	public Candidates getCandidates(@PathVariable int CandidatesCandidates_id) {
		Candidates theCandidate =candidatesService.findByIdC(CandidatesCandidates_id);
	...
	public Candidates addCandidates(@RequestBody Candidates theCandidate) {
			
	//also just in case the pass an id in JSON set the id =0
	// this is force the hibernate session to save new item instead of making update on it 	
		theCandidate.setCandidate_id(0);//if the id null or 0 DAO will insert new customer 
		
		
		candidatesService.saveC(theCandidate);
	...
	public Candidates updateCandidates(@RequestBody Candidates theCandidate) {
